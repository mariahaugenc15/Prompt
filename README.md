...
1. **Deploy the backend to [Render](https://render.com)** using the included blueprint:
   ...
   - `render.yaml` currently runs on the **free plan with no persistent disk** — accounts, uploaded photos, and everything else reset on every restart or redeploy. That's deliberate: it's the only way to get a real, live URL to test end-to-end without paying anything, and this app runs fine without a disk, it just doesn't remember anything between restarts.
   - When you're ready for data to actually survive a redeploy, upgrade the service to a plan that supports disks (Render's free tier can't attach one at all — the paid tier that includes one is called **Starter** on some accounts and **Pro** on others, ~$7–25/mo depending which your account offers), then uncomment the `disk:` block in `render.yaml` and add back a `DATA_DIR` env var set to `/var/data` (both are commented out in the file, with the exact steps inline). `server/db.ts` reads that path — without it, it falls back to a directory inside the container itself, which is what makes the no-disk mode above work at all.
   - **If this service wasn't created via New → Blueprint** (e.g. you clicked New → Web Service instead, or it predates this file), Render won't read `render.yaml` automatically — build command, start command, and env vars need to be set by hand in the dashboard (Settings → Build & Deploy, and the Environment tab) to match what's described here and in the file's comments.
